...
public class MapboxNavigation implements ServiceConnection {
    ...
    MapboxNavigation(@NonNull Context context, @NonNull String accessToken, LocationEngine locationEngine) {
        initializeContext(context);
        this.accessToken = accessToken;
    ...
    private void initialize() {
        ...
        if (options.defaultMilestonesEnabled()) {
            ...
            addMilestone(new BannerInstructionMilestone.Builder().setIdentifier(BANNER_INSTRUCTION_MILESTONE_ID)
                    .build());
        }
    }
